...
export const getAll = async () => {
  return await db.select().from(schema.achievements).orderBy(asc(schema.achievements.sortOrder));
};
...
export const create = async (data) => {
  if (data.sortOrder === undefined) {
    const existing = await db.select().from(schema.achievements).orderBy(asc(schema.achievements.sortOrder));
...
export const update = async (id, data) => {
  const result = await db.update(schema.achievements).set(data).where(eq(schema.achievements.id, id)).returning();
  return result.length > 0 ? result[0] : null;
};
